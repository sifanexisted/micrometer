...
def compute_dist_weights(coords, grid, eps):
    ...
    d2 = ((coords[:, :, jnp.newaxis, :] - grid[jnp.newaxis, :, :]) ** 2).sum(axis=3)
    # Compute weights using exponential kernel
    w = jnp.exp(-eps * d2) / jnp.exp(-eps * d2).sum(axis=2, keepdims=True)
    return w
...
class CViT(nn.Module):
    ...
    def __call__(self, x, coords):
        ...
        w = compute_dist_weights(coords, self.grid, self.eps)
        w = rearrange(
            w, "b n (h w) -> b n h w", h=self.grid_size[0], w=self.grid_size[1]
        )

        # Interpolate latents using weights
        coords = jnp.einsum("bhwc,bnhw->bnc", x, w)
